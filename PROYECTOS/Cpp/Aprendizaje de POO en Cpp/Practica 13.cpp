#include<iostream>
#include<math.h>
#include<stdlib.h>
#include<iomanip>
#include <windows.h>
#include<conio.h>

using namespace std;

class Datos{
public:
	void pedir();
private:
	int x;
	float y;
	char w;
};

void Datos::pedir(){
cout<<"Dame un numero entero 1: ";		cin>>x;
int *entero2 = new int(22);
int *entero3 = new int;
cout<<"Dame un numero entero 3: ";		cin>>*entero3;

cout<<"\nimprimir datos"<<endl;
cout<<" entero 1 = "<<x;
cout<<" entero 2 = "<<*entero2;
cout<<" entero 3 = "<<*entero3<<endl;

getch();


}

void gotoxy(int x, int y); 
void escribir();

int main(){
	
	system("COLOR 03");
	escribir();
	getch();
	system("cls");
	Beep(500, 100);
		
		Datos Varios;
		Varios.pedir();
		
	getch();
	system("cls");
	Beep(500, 100);
	return 0;
	Beep(500, 100);
	getch();
	system("pause");
}


void escribir(){
    gotoxy(45,11);cout<<"Instituto Politecnico Nacional";
    gotoxy(27,12);cout<<"Escuela Superior De Ingeneria Mecanica y Electrica UNIDAD CULHUACAN";
    gotoxy(44,13);cout<<"Monroy Cortines Jacobo Emiliano";
    gotoxy(43,14);cout<<"Programacion Orientada en Objetos";
    gotoxy(49,15);cout<<" Practica Numero 13";
    gotoxy(51,16);cout<<"  Grupo 2CV35";
    gotoxy(45,17);cout<<"Profesor Osornio Soto Roberto \n\n";  
}



void gotoxy(int x, int y){
    HANDLE hcon = GetStdHandle(STD_OUTPUT_HANDLE);
    COORD dwPos;
    dwPos.X = x;
    dwPos.Y = y;
    SetConsoleCursorPosition(hcon, dwPos);
}
